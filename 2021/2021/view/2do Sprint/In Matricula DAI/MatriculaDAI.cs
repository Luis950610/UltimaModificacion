...
namespace _2021
{
    public partial class MatriculasDAI : Form
    {
        ...
        {
            dgvMatriculasDAI.DataSource = oNegmatricula.N_Detalle_MatriculaDAI();
            /*dataGridView1.DataSource = objetoCN_Matricula.N_Detalle_EstudianteMatriculado()
        ...
        {/*
            if (cnbTipoDscto.Text == "DCTO1")
            {
                textCosto.Text = (float.Parse(textPago.Text) - 0).ToString();
            }
            else if (cnbTipoDscto.Text == "DCTO2")
            {
                textCosto.Text = (float.Parse(textPago.Text) -0).ToString();
        ...
        private void txtBuscarAlumnoMatriculado_TextChanged(object sender, EventArgs e)
        {
            if (txtBuscarAlumnoMatriculado.Text != "")
            {
                oEnt.CodCursoActivo = txtBuscarAlumnoMatriculado.Text;
                DataTable DT = new DataTable();
                DT = oNeg.N_BuscarAlumnoMatriculado(oEnt);
                dgvAlumnosMatriculados.DataSource = DT;
            }
